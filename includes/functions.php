...
session_destroy();
echo "You Were Killed Sign Back Up and Try To Get Revenge";
exit();
}
...
elseif (($info->rank == "Criminal") && ($info->rankpoints >= "1500")){ $newrank="Hitman"; $done="1"; }
elseif (($info->rank == "Hitman") && ($info->rankpoints >= "2500")){ $newrank="Wanted Criminal"; $done="1"; }
elseif (($info->rank == "Wanted Criminal") && ($info->rankpoints >= "5000")){ $newrank="Hired Gunner"; $done="1"; }
elseif (($info->rank == "Hired Gunner") && ($info->rankpoints >= "7000")){ $newrank="Assassin"; $done="1"; }
elseif (($info->rank == "Assassin") && ($info->rankpoints >= "9000")){ $newrank="Boss"; $done="1"; }
elseif (($info->rank == "Boss") && ($info->rankpoints >= "12000")){ $newrank="Don"; $done="1"; }
elseif (($info->rank == "Don") && ($info->rankpoints >= "14000")){ $newrank="Enemy of the State"; $done="1"; }
elseif (($info->rank == "Enemy of the State") && ($info->rankpoints >= "18000")){ $newrank="Global Dominator"; $done="1"; }
elseif (($info->rank == "Global Dominator") && ($info->rankpoints >= "25000")){ $newrank="Legend"; $done="1"; }
elseif (($info->rank == "Legend") && ($info->rankpoints >= "60000")){ $newrank="Legendary Legend"; $done="1"; }
...
VALUES (
'', '$username', '$username', 'Congratulations! You have been promoted to $newrank! Well Done!', '$date', '0', '0', '0'
)");
...
if ($info->rank == "Wannabe"){ $new_res="1"; }elseif($info->rank == "Goon"){  $new_res="2"; }elseif($info->rank == "Hired Thug"){  $new_res="3"; }elseif($info->rank == "Criminal"){  $new_res="4";    }elseif($info->rank == "Hitman"){  $new_res="5";   }elseif($info->rank == "Wanted Criminal"){  $new_res="6";  }elseif($info->rank == "Hired Gunner"){  $new_res="7";
}elseif($info->rank == "Assassin"){  $new_res="8"; }elseif($info->rank == "Boss"){  $new_res="9"; }elseif($info->rank == "Don"){  $new_res="10"; }elseif($info->rank == "Enemy of the State"){  $new_res="11"; }elseif($info->rank == "Global Dominator"){  $new_res="12"; }elseif($info->rank == "Legend"){  $new_res="13"; }elseif($info->rank == "Legendary Legend"){  $new_res="14"; }
