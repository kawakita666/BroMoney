...
<table width='68%' border='1' align='center' cellpadding='0' cellspacing='0' class=thinline rules=none>
  ...
  <tr>
    <td><table width='100%' border='0' cellspacing='3' cellpadding='0'>
        <tr>
          <td colspan='3'>".replace($get->message)."</td>
        </tr>";
if ($get->witness == "1"){ echo "<tr><td>Witness ID: $get->id</td></tr>"; }

echo "<tr>
          <td width='6%' height='23'><input type=\"checkbox\" name=\"radio[]\" value=\"$get->id\"></form></td>
          <td width='7%'> <form name='form2' method='post' action='?s=$get->id'>
              <input type='submit' name='Submit' value='Save'>
          </td>
          <td width='87%'><form name='form1' method='post' action='?del=$get->id'><input name='Delete' type='submit' id='Delete' value='Delete'></form>
		  <form action=send.php?rep=$get->id method=POST><input type=submit value=Reply></form></td>
